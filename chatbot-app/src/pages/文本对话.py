...
def chat_page():
    ...
    model_name = st.selectbox('模型选择', st.session_state.model_list, key='chat_model_name')

    # 系统提示词选项
    option = st.radio("系统提示语", ("手动输入", "提示语"), horizontal=True, index=0)
    if option == "手动输入":
        system_prompt = st.text_input('系统提示（修改后请点击“清除历史”按钮.）',
...
def num_tokens_from_messages(messages, model):
    ...
    if model in {
        ...
        # 每个消息有一个基本的令牌数 tokens_per_message，默认 3 个 token，每个 name 属性预设的固定令牌数 tokens_per_name，假设其值为 1。
