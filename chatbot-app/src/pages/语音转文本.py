...
def stt_page():
    ...
    # 选择输入方式：录音或上传
    option = st.radio("输入方式:", ("录音", "上传"), horizontal=True, index=0)
    if option == "录音":
    ...
    # 如果用户选择上传方式
        ...
        st.write("注意!当前 OpenAI 接口仅支持最大 25MB 的文件.")
        # 创建文件上传控件，允许用户上传音频文件。
        audio_file = st.file_uploader("上传音频文件", type=["wav", "mp3", "webm", "mpeg", "mpga"])
        # 如果用户上传了文件
        if audio_file:
            # 检查文件大小是否超过最大限制（25MB）
            if audio_file.size > MAX_FILE_SIZE:
                # 如果文件过大，显示错误信息。
                st.error("上传的文件过大。请上传小于25MB的音频文件.")
